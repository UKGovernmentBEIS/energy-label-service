...
public class RefrigeratingAppliancesController extends CategoryController {
  ...
  public RefrigeratingAppliancesController(RefrigeratingAppliancesService householdRefrigeratingAppliancesService,
    ...
    super(BREADCRUMB_STAGE_TEXT, breadcrumbService, RefrigeratingAppliancesCategory.GET,
        RefrigeratingAppliancesController.class);
  ...
  @ResponseBody
  public Object handleFridgesFreezersSubmit(@Valid @ModelAttribute("form") FridgesFreezersForm form,
  ...
  @ResponseBody
  public Object handleInternetLabelFridgesFreezersSubmit(
      @Validated({InternetLabellingGroup.class, RescaledInternetLabellingGroup.class}) @ModelAttribute("form") FridgesFreezersForm form,
      BindingResult bindingResult) {
    if (bindingResult.hasErrors()) {
      return getFridgesFreezers(bindingResult.getFieldErrors());
    } else {
  ...
  @ResponseBody
  public Object handleWineStorageAppliancesSubmit(@Valid @ModelAttribute("form") WineStorageAppliancesForm form,
                                                  BindingResult bindingResult) {
    if (bindingResult.hasErrors()) {
  ...
  @ResponseBody
  public Object handleInternetLabelWineStorageAppliancesSubmit(
      @Validated({InternetLabellingGroup.class, RescaledInternetLabellingGroup.class}) @ModelAttribute("form") WineStorageAppliancesForm form,
      BindingResult bindingResult) {
    if (bindingResult.hasErrors()) {
      return getWineStorageAppliances(bindingResult.getFieldErrors());
    } else {
  ...
  private void addCommonObjects(ModelAndView modelAndView, List<FieldError> errorList, String submitUrl) {
    modelAndView.addObject("efficiencyRating", ControllerUtils.ratingRangeToSelectionMap(
        RefrigeratingAppliancesService.LEGISLATION_CATEGORY_CURRENT.getPrimaryRatingRange()));
    modelAndView.addObject("noiseEmissionsRating", ControllerUtils.ratingRangeToSelectionMap(
        RefrigeratingAppliancesService.LEGISLATION_CATEGORY_CURRENT.getSecondaryRatingRange()));
